...
export const getEvents = async (req, res) => {
  try {
    const { bdeId, limit = 50, upcoming = false } = req.query;
  ...
  try {
    ...
    if (event.status !== 'PUBLISHED') {
      ...
      if (req.user.role !== 'admin_interasso') {
        ...
        if (req.user.role !== 'admin_bde' || req.user.bdeId?.toString() !== event.bdeId._id.toString()) {
          return res.status(403).json({
            success: false,
            error: 'Accès refusé - Événement non publié'
          });
...
export const createEvent = async (req, res) => {
  try {
    ...
    console.log(`✨ Nouvel événement créé: "${event.title}" (${event.bdeId.name}) - Statut: PENDING`);
  ...
  try {
    ...
    if (req.user.role === 'admin_bde') {
      ...
      if (event.bdeId.toString() !== req.user.bdeId.toString()) {
        return res.status(403).json({
          success: false,
          error: 'Accès refusé - Vous ne pouvez modifier que vos propres événements'
        });
      }
      ...
      if (event.status !== 'PENDING') {
        return res.status(403).json({
          success: false,
          error: 'Accès refusé - Vous ne pouvez modifier que les événements en attente'
        });
      }
  ...
  try {
    ...
    if (req.user.role === 'admin_bde') {
      ...
      if (event.bdeId.toString() !== req.user.bdeId.toString()) {
        return res.status(403).json({
          success: false,
          error: 'Accès refusé - Vous ne pouvez supprimer que vos propres événements'
        });
      }
    } else if (req.user.role !== 'admin_interasso') {
      return res.status(403).json({
