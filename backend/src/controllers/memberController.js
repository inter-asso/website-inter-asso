...
export const createMember = async (req, res) => {
  try {
    if (req.user.role !== 'admin_bde') {
      return res.status(403).json({
        success: false,
        error: 'Accès refusé - Seuls les administrateurs BDE peuvent créer des membres'
      });
    }
    ...
    console.log(`✨ Nouveau membre créé: ${member.firstName} ${member.lastName} (${member.bdeId.name})`);
...
export const updateMember = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin_bde') {
      return res.status(403).json({
        success: false,
        error: 'Accès refusé - Seuls les administrateurs BDE peuvent modifier des membres'
      });
    }
    ...
    if (member.bdeId.toString() !== req.user.bdeId.toString()) {
      return res.status(403).json({
        success: false,
        error: 'Accès refusé - Vous ne pouvez modifier que les membres de votre bureau'
      });
    }
    ...
    console.log(`✏️ Membre modifié: ${member.firstName} ${member.lastName} par ${req.user.email}`);

    res.json({
      success: true,
      message: 'Membre modifié avec succès',
      member: await Member.findById(id).populate('bdeId', 'name slug logo colors')
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const deleteMember = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin_bde') {
      return res.status(403).json({
        success: false,
        error: 'Accès refusé - Seuls les administrateurs BDE peuvent supprimer des membres'
      });
    }
    ...
    if (member.bdeId.toString() !== req.user.bdeId.toString()) {
      return res.status(403).json({
        success: false,
        error: 'Accès refusé - Vous ne pouvez supprimer que les membres de votre bureau'
      });
    }
    ...
    console.log(`🗑️ Membre supprimé: ${member.firstName} ${member.lastName} par ${req.user.email}`);
